...
- **로컬 백업** (`services/backup/`): DB manifest(JSON) + 사진 원본/썸네일을 ZIP 한 개로 묶음(`local_backup_service.dart`, `database_backup.dart`). 복원은 향후.
...
- 클라우드 백업·소셜 로그인·AdMob은 **미연동**(스캐폴드만 존재).
...
- 소셜 로그인(Google/Apple) + 클라우드 자동 백업(Drive/iCloud) — `account_repository`/`backup/*` 스캐폴드 활용.
